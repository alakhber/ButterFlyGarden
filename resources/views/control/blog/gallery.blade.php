...
@section('title', 'Admin Dashboard | ' . $blog->name . '| Qalerya ')
@section('_styles')
    ...
    <script>
        $(function() {
            ...
            function saveNewPosition() {
                ...
                $.ajax({
                    url: `{{ route('system.blog.gallery.sortable', $blog) }}`,
                    method: 'POST',
                    data: {
                        _token: `{{ csrf_token() }}`,
                        _method: 'PUT',
...
@section('_content')
    <div class="page-header page-header-light">
        <div class="page-header-content d-sm-flex">
            <div class="page-title">
                <h4><span class="font-weight-semibold">{{ $blog->name }} Üçün Şəkillər</span></h4>
            </div>
        </div>
        @if ($errors->any())
        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-sm-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <a href="{{ route('system.blog.index') }}" class="breadcrumb-item"> Məhsullar</a>
                    <a href="{{ route('system.blog.show',$blog) }}" class="breadcrumb-item"> {{ $blog->name }}</a>
    ...
    <div class="content">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('system.blog.gallery.upload', $blog) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                                                ...
                                                <ul class="el-info">
                                                    ...
                                                    <li>
                                                        <a title="Sil" href="javascript:void(0)" class="btn default btn-outline DELETEITEM "><i class="text-danger fas fa-trash"></i>
                                                            <form action="{{ route('system.blog.gallery.delete', ['blog' => $blog, 'image' => $image]) }}" method="post">
                                                                @csrf
                                                                @method('delete')
                                                            </form>
                                                        </a>
